import type { Route, GoalRoute, Tracker, SlotData, Datapackage, Filter } from './types'
...
export function enumerateGoalRoutes(
  ...
  for (const cp of goalDef.checkpoints) {
    ...
    if (cpItem !== null && !(checkpointsanity && goalCheckpointsanity)) continue
    ...
    const perRoomOptions: Route[][] = cp.rooms.map(roomRoutes => {
      const valid = roomRoutes.filter(r => !routeIsBlocked(inventory, r, keysanity))
...
export function enumerateAllRoutes(
  ...
  for (let i = 0; i < numPlayers; i++) {
    ...
    if (inventory.has("Granny's House Keys")) continue
    ...
    const freeItems = new Set<string>(keysanity ? [] : (GOAL_KEYS[goalArea] ?? []))

    const routes = enumerateGoalRoutes(
      slotName, goalArea, inventory, slotData,
      checkpointsanity, goalCheckpointsanity, freeItems, keysanity
    )

    // Apply threshold filter
    const viable = filter.mode === 'threshold'
